...
% Sensible defaults seeded into the metric table on first run.
    specs = [ ...
        ...
        ms('HRV',            's',            '_HRVMeasures.mat',  'hrv',                'auto',   NaN); ...
        ms('pNN5',           '%',            '_HRVMeasures.mat',  'pnn5',               'auto',   NaN); ...
        ms('RMSSD',          's',            '_HRVMeasures.mat',  'rmssd',              'auto',   NaN); ...
        ms('Sample entropy', '',             '_HRVMeasures.mat',  'sampEn',             'auto',   NaN); ...
        ms('SD1',            's',            '_HRVMeasures.mat',  'sd1',                'auto',   NaN); ...
        ms('SD2',            's',            '_HRVMeasures.mat',  'sd2',                'auto',   NaN); ...
...
function s = ms(label, units, suffix, field, aggregator, channel)
    s = struct('label',label,'units',units,'suffix',suffix,'field',field, ...
               'aggregator',aggregator,'channel',channel);
end

function s = displayLabel(spec)
% '<label> (<units>)' if units non-empty, else bare label.
    lab = strtrim(char(getf(spec,'label')));
    un  = strtrim(char(getf(spec,'units')));
