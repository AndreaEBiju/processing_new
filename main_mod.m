...
SDKPATH = 'G:/Shared drives/BIONICs Lab Workspace/Project Folders/GEMS/Survivals/Chronic Rat A - 11-22-25/mstim sweep 11-26-25/ME_STIM-251126_2/TDTMatlabSDK'; % or whatever path you extracted the SDK zip into
...
for folderidx = 2:length(folderNames)
...
%% remove artifacts
for folderidx = 6:6%1:length(folderNames)
    folderPath = folderPaths{folderidx};
    condition = string(folderNames{folderidx});
...
for folderidx = 1%[7:17,19:20]%1:length(folderNames)
    ...
    if exist('stim_y','var') && exist('rec_y','var')
        ...
        results = HR_BR_HRVAnalysis_new(data, fs, cutoff, order, folderPath, condition, hrChanIdx, 'stim_rec', blankIdx, 0.75, 20, 1, figtrue);

        swResults = slowWaveAnalysis_new(data(3:5), lowPassOn, lowPassCutoff, ...
    ...
    elseif exist('y','var')
        ...
        swResults = slowWaveAnalysis_new( ... 
